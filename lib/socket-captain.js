...
module.exports = function createCaptainSocket(serviceLocator) {
  ...
  function run() {
    var admiralProtocol = serviceLocator.config.admiral && serviceLocator.config.admiral.protocol || 'http'
      , admiralHost = serviceLocator.config.admiral && serviceLocator.config.admiral.host || '127.0.0.1'
      ...
      , connectionString =
          admiralProtocol + '://' + admiralUser + ':' + admiralPassword + '@' + admiralHost + ':' + admiralPort
