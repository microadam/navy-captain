module.exports = function orderExecuter(serviceLocator) {
  ...
  function execute(data, client, callback) {
    ...
    // TODO If we dont recognise the order, just say its been successful. Is this the right approach?
    if (!order) {
      return callback({ success: true })
    }

    var steps = order.getSteps()
      ...
      , stepContext =
        ...
        , emit: emit
        , isMaster: serviceLocator.stateHandler.getMasterState(data.appId, data.environment)
